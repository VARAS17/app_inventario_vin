<div class="min-h-screen bg-zinc-100 text-zinc-800 text-xs font-sans p-4 space-y-4">
    ...
    <div class="bg-white border border-zinc-300 p-3 flex flex-col md:flex-row items-center justify-between gap-3 shadow-none rounded-none">
        ...
        <div class="flex items-center gap-3 w-full md:w-auto">
            ...
            <button type="button" 
                ...
                <span>Exportar Mantenimientos</span>
            </button>
        </div>
    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-zinc-200">
                            @forelse ($mantenimientos as $item)
                                @php $isSelected = ($selectedMantenimientoId === $item->id); @endphp
                                <tr wire:key="mant-{{ $item->id }}" 
                                    ...
                                    <td class="p-2.5 uppercase font-medium text-zinc-700 whitespace-nowrap">
                                        {{ $item->areaOrigen->nombre ?? 'ALMACEN' }}
                                    </td>
                                    <td class="p-2.5 font-mono text-zinc-600 whitespace-nowrap">
                                        {{ $item->fecha_envio ? \Carbon\Carbon::parse($item->fecha_envio)->format('d-m-Y') : '—' }}
                                    </td>
                                    <td class="p-2.5 font-mono whitespace-nowrap">
                                        @if ($item->fecha_ingreso)
                                            <span class="text-emerald-700 font-semibold">{{ \Carbon\Carbon::parse($item->fecha_ingreso)->format('d-m-Y') }}</span>
                                        @else
                                    ...
                                    <td class="p-2.5 text-right whitespace-nowrap" wire:click.stop>
                                        <div class="inline-flex items-center gap-1">
                                            ...
                                            <!-- ÚNICO BOTÓN EDITAR / GESTIONAR (LÁPIZ) -->
                                            <button type="button" 
                                                    wire:click.stop="abrirModalEditar({{ $item->id }})" 
                                                    title="Editar datos o registrar retorno"
            ...
            <div class="col-span-12 lg:col-span-5 xl:col-span-4 bg-white border border-zinc-300 rounded-none space-y-0 shadow-none">
                ...
                <div class="bg-zinc-900 text-zinc-100 p-2.5 flex items-center justify-between uppercase font-mono text-[11px] tracking-wider border-b border-zinc-300">
                    ...
                    <button type="button" 
                            ...
                            class="text-zinc-400 hover:text-white font-bold px-1.5 py-0.5 border border-zinc-700 hover:border-zinc-500 cursor-pointer">
                        ✕ CERRAR DETALLE
                    </button>
                </div>
                ...
                <div class="p-3.5 border-b border-zinc-300 space-y-3 bg-zinc-50/50">
                    <div class="text-[11px] font-bold uppercase tracking-wider text-zinc-600 border-b border-zinc-200 pb-1 flex justify-between items-center">
                        ...
                        <span class="font-mono text-[10px] px-1.5 py-0.5 border {{ $tec->estado === 'En Mantenimiento' ? 'bg-amber-100 text-amber-800 border-amber-300' : ($tec->estado === 'De baja' ? 'bg-rose-100 text-rose-800 border-rose-300' : 'bg-emerald-100 text-emerald-800 border-emerald-300') }}">
                            {{ $tec->estado }}
                        </span>
                    </div>
                    ...
                    <div class="flex gap-3 items-start">
                        ...
                        <div class="space-y-1 text-[11px] w-full">
                            ...
                            <div class="grid grid-cols-2 gap-1 pt-1">
                                ...
                                <div>
                                    <span class="text-zinc-500 uppercase block text-[10px]">Estado Previo:</span>
                                    <span class="text-zinc-800 font-bold uppercase">{{ $mant->estado_previo ?? 'N/A' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="p-3.5 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-2 text-[11px]">
                        <div>
                            ...
                            <span class="font-medium text-zinc-900 uppercase">{{ $mant->areaOrigen->nombre ?? 'ALMACEN' }}</span>
                        </div>
                        <div>
                            <span class="text-zinc-500 uppercase block text-[10px]">Tipo / Técnico:</span>
                            <span class="font-medium text-zinc-900">{{ $mant->tipo }} {{ $mant->taller_proveedor ? '('.$mant->taller_proveedor.')' : '' }}</span>
                        </div>
                        <div>
                            <span class="text-zinc-500 uppercase block text-[10px]">Fecha de Envío:</span>
                            <span class="font-mono text-zinc-900">{{ $mant->fecha_envio ? \Carbon\Carbon::parse($mant->fecha_envio)->format('d-m-Y') : '—' }}</span>
                        </div>
                        <div>
                            <span class="text-zinc-500 uppercase block text-[10px]">Fecha de Retorno:</span>
                            <span class="font-mono text-zinc-900">
                                {{ $mant->fecha_ingreso ? \Carbon\Carbon::parse($mant->fecha_ingreso)->format('d-m-Y') : 'PENDIENTE' }}
                            </span>
                        </div>
                        <div class="col-span-2">
                            <span class="text-zinc-500 uppercase block text-[10px]">Motivo / Falla Reportada:</span>
                        ...
                        @if ($mant->solucion)
                            <div class="col-span-2">
                                <span class="text-zinc-500 uppercase block text-[10px]">Trabajo Realizado / Diagnóstico:</span>
                                <p class="p-2 bg-zinc-100 border border-zinc-200 text-zinc-800 text-[11px] leading-relaxed">
                                    {{ $mant->solucion }}
                                </p>
                                <div class="mt-1 text-[10px] font-mono">
                                    Condición de Retorno: 
                                    @if ($mant->quedo_operativo)
                                        <span class="text-emerald-700 font-bold uppercase">✔ Operativo</span>
                                    @else
                                        <span class="text-rose-700 font-bold uppercase">✖ Irreparable (De baja)</span>
                                    @endif
                                </div>
                            </div>
                        @endif
                            ...
                            @forelse ($mant->archivos as $arch)
                                <div class="p-2 bg-zinc-50 border border-zinc-300 flex items-center justify-between hover:bg-zinc-100 transition">
                                    ...
                                    <button type="button" 
                                            ...
                                            class="px-2 py-1 bg-white border border-zinc-300 hover:border-blue-700 text-zinc-700 hover:text-blue-700 shrink-0 font-semibold text-[10px] flex items-center gap-1 rounded-none cursor-pointer">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- ========================================================== -->
    <!-- 3. MODAL: FORMULARIO INTEGRADO (CON RECUADRO VERDE)       -->
    <!-- ========================================================== -->
    @if ($modalFormulario)
            ...
            <div class="bg-white border border-zinc-400 w-full max-w-2xl rounded-none shadow-none text-xs space-y-0">
                ...
                <div class="bg-zinc-900 text-zinc-100 p-3 flex items-center justify-between uppercase font-mono text-[11px] tracking-wider">
                    <span class="font-bold flex items-center gap-2">
                        ...
                        {{ $mantenimiento_id ? '[EDITAR / RETORNO] EXPEDIENTE DE MANTENIMIENTO' : '[NUEVO] ENVIAR ACTIVO A MANTENIMIENTO' }}
                    </span>
                    <button type="button" wire:click="$set('modalFormulario', false)" class="text-zinc-400 hover:text-white font-bold cursor-pointer">&times;</button>
                </div>

                <form wire:submit.prevent="guardar" class="p-4 space-y-4">

                    <!-- SECCIÓN 1: DATOS DE SALIDA AL TALLER -->
                    <div class="space-y-1.5">
                        ...
                        @if ($tecnologiaSeleccionada)
                            @php $fotoModal = $this->getFotoBase64($tecnologiaSeleccionada->foto); @endphp
                            <div class="p-2.5 bg-zinc-50 border border-zinc-300 flex items-center justify-between">
                                ...
                                @if (!$mantenimiento_id)
                                    <button type="button" 
                                            ...
                                            class="px-2 py-1 bg-white border border-zinc-300 hover:border-zinc-500 font-semibold uppercase text-[10px] tracking-wider rounded-none cursor-pointer">
                                        Cambiar
                                    </button>
                                @endif
                            </div>
                        @else
                            <div class="relative">
                                <input type="text" 
                                       ...
                                       placeholder="Escriba VIN, nombre o serie para buscar activo..." 
                                ...
                                @if ($this->tecnologiasEncontradas->count() > 0)
                                    <div class="absolute left-0 right-0 top-full bg-white border border-zinc-300 z-10 max-h-48 overflow-y-auto divide-y divide-zinc-200 shadow-sm">
                                        @foreach ($this->tecnologiasEncontradas as $itemTec)
                                            @php $miniFoto = $this->getFotoBase64($itemTec->foto); @endphp
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <div>
                            <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                Tipo de Mantenimiento <span class="text-red-600">*</span>
                            </label>
                            <select wire:model="tipo" class="w-full bg-zinc-50 border border-zinc-300 p-2 text-xs rounded-none focus:border-blue-700 focus:bg-white focus:ring-0">
                                <option value="Correctivo">Correctivo (Reparación de Falla)</option>
                                <option value="Preventivo">Preventivo (Limpieza / Servicio)</option>
                            </select>
                            @error('tipo') <span class="text-[11px] text-red-600 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                Técnico / Taller Responsable <span class="text-zinc-400 font-normal lowercase">(opcional)</span>
                            </label>
                            <input type="text" wire:model="taller_proveedor" placeholder="Ej: Soporte TI, Taller Externo..." class="w-full bg-zinc-50 border border-zinc-300 p-2 text-xs rounded-none focus:border-blue-700 focus:bg-white focus:ring-0">
                            @error('taller_proveedor') <span class="text-[11px] text-red-600 block">{{ $message }}</span> @enderror
                        </div>

                        <div>
                            <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                Fecha de Envío al Taller <span class="text-red-600">*</span>
                            </label>
                        ...
                        <div class="md:col-span-2">
                            <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                Motivo o Síntoma de Falla <span class="text-red-600">*</span>
                            </label>
                            <textarea wire:model="motivo" rows="2" placeholder="Describa los problemas técnicos por los que se envía el equipo..." class="w-full bg-zinc-50 border border-zinc-300 p-2 text-xs rounded-none focus:border-blue-700 focus:bg-white focus:ring-0"></textarea>
                            @error('motivo') <span class="text-[11px] text-red-600 block">{{ $message }}</span> @enderror
                        </div>
                    </div>

                    <!-- ============================================================== -->
                    <!-- SECCIÓN 2: RECUADRO VERDE (LLENAR CUANDO EL EQUIPO RETORNE)    -->
                    <!-- ============================================================== -->
                    <div class="border-2 border-emerald-600 bg-emerald-50/50 p-3.5 space-y-3">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <div>
                                <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                    Fecha de Retorno a Oficina <span class="text-zinc-400 font-normal lowercase">(al volver)</span>
                                </label>
                                <input type="date" wire:model="fecha_ingreso" class="w-full bg-white border border-zinc-300 p-2 text-xs font-mono rounded-none focus:border-blue-700 focus:ring-0">
                                @error('fecha_ingreso') <span class="text-[11px] text-red-600 block">{{ $message }}</span> @enderror
                            </div>

                            <div class="flex items-center pt-4">
                                <label class="flex items-center gap-2 cursor-pointer bg-white p-2 border border-zinc-300 w-full">
                                    <input type="checkbox" wire:model="quedo_operativo" class="rounded-none border-zinc-300 text-blue-700 focus:ring-0">
                                    <span class="text-[11px] font-semibold text-zinc-800">El activo quedó 100% operativo</span>
                                </label>
                            </div>

                            <div class="md:col-span-2">
                                <label class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider mb-1">
                                    Trabajo Realizado / Solución Técnica <span class="text-zinc-400 font-normal lowercase">(diagnóstico)</span>
                                </label>
                                <textarea wire:model="solucion" rows="2" placeholder="Describa el mantenimiento realizado, piezas cambiadas o diagnóstico final del técnico..." class="w-full bg-white border border-zinc-300 p-2 text-xs rounded-none focus:border-blue-700 focus:ring-0"></textarea>
                                @error('solucion') <span class="text-[11px] text-red-600 block">{{ $message }}</span> @enderror
                                <span class="text-[10px] text-zinc-500 font-mono block mt-0.5">(Si el equipo no tiene arreglo, desmarque la casilla superior para enviarlo a "De baja").</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="border-t border-zinc-300 pt-3 space-y-2">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="block uppercase font-bold text-[10px] text-zinc-700 tracking-wider">
                                    Guías de Envío / Informes Técnicos de Taller
                                </span>
                                <span class="text-[10px] text-zinc-500">Carga incremental de archivos físicos (PDF / Imágenes)</span>
                            </div>
                    ...
                    <div class="border-t border-zinc-300 pt-3 flex justify-end gap-2">
                        <button type="button" wire:click="$set('modalFormulario', false)" class="px-3 py-1.5 bg-white border border-zinc-300 hover:bg-zinc-100 font-semibold uppercase text-xs tracking-wider rounded-none cursor-pointer">
                            Cancelar
                        </button>
                        <button type="submit" wire:loading.attr="disabled" class="px-4 py-1.5 bg-blue-700 hover:bg-blue-800 text-white font-semibold uppercase text-xs tracking-wider rounded-none disabled:opacity-50 cursor-pointer">
                            <span wire:loading.remove wire:target="guardar">
                                {{ $mantenimiento_id ? 'Guardar Cambios' : 'Registrar Salida a Taller' }}
                            </span>
            ...
            <div class="bg-white border border-red-500 w-full max-w-md rounded-none shadow-none text-xs space-y-0">
                ...
                <div class="p-4 space-y-3 bg-zinc-50">
                    ...
                    <div class="p-2.5 bg-red-50 border border-red-200 text-red-800 text-[11px] font-mono space-y-1">
                        ...
                        <span>Se destruirán de forma irrevocable todos los archivos locales vinculados a este registro.</span>
                    </div>
                </div>
                ...
                <div class="p-3 bg-white border-t border-zinc-300 flex justify-end gap-2">
                    ...
                    <button type="button" wire:click="eliminarMantenimiento" class="px-3 py-1.5 bg-red-700 hover:bg-red-800 text-white font-semibold uppercase text-xs tracking-wider rounded-none cursor-pointer">
